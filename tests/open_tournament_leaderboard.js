...
import { GET_TOURNAMENTS, GET_LEADERBOARD_OF_TOURNAMENT } from '../config/constants.js';
import { tournament_id_for_leaderboard } from '../config/constants.js';
...
export default function () {
    group('validate Opening all tournaments list', function () {
        ...
        get_tournament_leaderboard(selectedTournamentId);
    });
}

const get_tournament_leaderboard = id => {
    group('validate Opening one tournament from list', function () {
        console.log("Opening one tournament from list");

        let tournament_ID = id;

        const res = http.get(GET_LEADERBOARD_OF_TOURNAMENT + '/' + tournament_ID);
        ...
        if (responseBody.message && responseBody.message.startsWith("Unable to find records for tournament_id")) {
            ...
            tournament_ID = tournament_id_for_leaderboard; // Assign the backup tournament ID

            const resBackup = http.get(GET_LEADERBOARD_OF_TOURNAMENT + '/' + tournament_ID);
...
export function handleSummary(data) {
    return createHtmlReport(data, 'open_tournament_leaderboard')
}