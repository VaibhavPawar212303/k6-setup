...
import { GET_TOURNAMENTS, GET_MATCHPLAY_SUMMARY, GET_PLAYER_OF_TOURNAMENT, SEARCH_FOR_PLAYER } from '../config/constants.js';
...
const get_tournament_players = (tournamentId, roundId) => {
    group('validate Opening tournaments player', function () {
        ...
        if (responseBody.message && responseBody.message.startsWith("Unable to find records for tournament_id")) {
            ...
            if (players && players.leaderboard && players.leaderboard.results) {
                ...
                search_for_player(selectedPlayerId)
            } else {
...
const search_for_player = id => {
    group('validate searching for player', function () {
        console.log("searching for player");

        let player_Id = id;

        const res = http.get(SEARCH_FOR_PLAYER + '?=' + player_Id);
...
export function handleSummary(data) {
    return createHtmlReport(data, 'Search_player')
}